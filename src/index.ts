...
import { displayTimerStatus, formatStopwatchTime, removeLeadingZeroAndConvertIntoNumber, validateFormat } from './utils';
...
async function start() {
	...
	if (choice === "timer") {
		...
		const time = await input({
			...
			validate: (val) => validateFormat(val) || "You must enter in correct format. minutes:seconds"
		});
		...
		const additionalHours = Math.floor((originalMinutes + remainingMinutes) / 60);
		...
		const timeInterval = setInterval(() => {
			...
			displayTimerStatus(remainingMinutes, remainingSeconds, newHours, newMinutes, endDays)
